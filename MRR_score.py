...
def MRR_score_for_data(model, data_loader, batch_size):
    ...
    for i, data_pack in enumerate(data_loader, 0):
        ...
        for seq_idx, a_sorted_seq in enumerate(indices):
            sum_of_rank_score = 0
            for rank, idx in enumerate(a_sorted_seq):
                if (y_[seq_idx, idx] == 1.0):
                    rank_score = 1/(rank+1)
                    sum_of_rank_score += rank_score

            target_basket_size = (y_[seq_idx] != 0.0).sum(dim=-1)
            MRR_score = sum_of_rank_score/target_basket_size.item()
