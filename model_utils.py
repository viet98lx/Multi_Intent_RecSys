...
def train_model(model, loss_func, optimizer, train_loader, epoch, top_k, train_display_step):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        predict = model(x_train_batch, train_seq_len)  # predicted output
        ...
        train_recall_item = utils.compute_recall_at_top_k(model, predict, top_k, y_train, real_batch_size, device)
        ...
        if ((i + 1) % train_display_step == 0 or (i + 1) == total_train_batch):  # print every 50 mini-batches
            top_pred = predict.detach().topk(dim=-1, k=model.top_k, sorted=True)
            print(
